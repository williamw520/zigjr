...
TODOs
* DONE Add dispatcher.free() to free on the RunResult. Let the dispatcher handle freeing memory.
...
* DONE Add batch dispatching in runner.
...
* TODO Rename run() to dispatch() on dispatcher.
* TODO Compact JSON output.
