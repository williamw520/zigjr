...
** DONE Add network server example.
** DONE Migrate allocator in RpcHandler to caller, to allow for per-thread allocator instead of global allocator in the RpcHandler.
** DONE Add DispatchCtxImpl with user type support to handler.
** TODO Have a separate DispatchCtxImpl for RpcDispatcher/Dispatcher and a separate DispatchCtx(R) for RpcHandler.
** TODO Add per-request user data type to DispatchCtxImpl and RpcHandler.
** TODO Per-request dispatch context.
*** TODO Per-request dispatch context setup on onBefore.
*** TODO Per-request dispatch context cleanup on onEnd.
