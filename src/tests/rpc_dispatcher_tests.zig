...
test "rpc_dispatcher fn0 and variants with DispatchCtx" {
...

    {
        ...
        dc.request = &req;
        const d_result = try req_dispatcher.dispatch(&dc);
        _=d_result;
...
test "rpc_dispatcher fn1 and variants with DispatchCtx" {
...

    {
        ...
        const req = try parsed_req.request();
        dc.request = &req;
        _ = try req_dispatcher.dispatch(&dc);
...
test "rpc_dispatcher - pipeline - fn0" {
    var gpa = std.heap.DebugAllocator(.{}){};
    defer _ = gpa.deinit();
    const alloc = gpa.allocator();
...
test "rpc_dispatcher extended handlers" {
...

    {
        ...
        try dispatcher.add(zigjr.H_PRE_REQUEST, ExtHandlers.onBefore);
        try dispatcher.add(zigjr.H_END_REQUEST, ExtHandlers.onAfter);
        try dispatcher.add(zigjr.H_ON_ERROR, ExtHandlers.onError);
        try dispatcher.add(zigjr.H_FALLBACK, ExtHandlers.onFallback);
